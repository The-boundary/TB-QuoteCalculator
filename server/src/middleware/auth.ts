...
const getAllowedDomains = (): string[] => {
  const raw = (process.env.GOOGLE_WORKSPACE_DOMAIN || 'the-boundary.com')
    .split(',')
...
const extractToken = (req: Request): string | null => {
  ...
  const cookieToken = cookies?.tb_access_token;
...
const verifySupabaseToken = (token: string): any => {
...
const ensureAppAccess = async (userId: string): Promise<AppAccess> => {
  const accessSupabase = getAuthSupabaseClient();
...
export const requireAuth = async (
  ...
  try {
    ...
    const providedServiceToken = Array.isArray(rawServiceToken)
      ? rawServiceToken[0]
      : rawServiceToken;
